 ...
 */
class CProducer implements Producer {

    private final T01Generator gen;

    public CProducer(T01Generator gen) {
        this.gen = gen;
    }

    @Override
    public Set<String> produce(Set<String> input) {
        if (!gen.getFreq().keySet().contains("C")) {
            return input;
        }

        int count = gen.getFreq().get("C");
        int unique = gen.getCandidates().get("C").size();
        int[] hit = new int[unique];
        String[] candidates = new String[unique];
        int pos = 0;
        for (String op : gen.getCandidates().get("C")) {
            candidates[pos++] = op;
        }
